...
class BleProvider extends ChangeNotifier {
  ...
  void _setupSubscriptions() {
    ...
    _connectionStatusSubscription = _bleService.connectionStatusStream.listen((status) {
      _connectionStatus = status;
      
      if (status == 'FALL_DETECTED') {
        _handleFallDetection();
      } else if (status.contains('Disconnected') && _autoReconnectEnabled && _savedDeviceId != null) {
  ...
  Future<void> _attemptReconnect() async {
    if (_savedDeviceId != null && !isConnected) {
      ...
      final savedDevice = _discoveredDevices.where(
    ...
    if (_emergencyContactProvider != null) {
      try {
        ...
        await _emergencyContactProvider!.sendEmergencyAlert(
          isTest: false,
          customMessage: 'FALL DETECTED! LifeGuard device has detected a potential fall. Please check on the user immediately.',
          location: location,
        );
