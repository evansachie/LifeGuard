...
export function BLEProvider({ children }) {
  ...
  const connectToDevice = useCallback(async () => {
    try {
      ...
      if (validUUIDs.length !== characteristicUUIDs.length) {
        console.error(
          'Some UUIDs are undefined:',
          characteristicUUIDs.map((uuid, i) => `${Object.keys(BLE_UUID)[i + 1]}: ${uuid}`)
        );
  ...
  const startDataReading = useCallback(async (characteristics) => {
    const [tempChar, humChar, pressChar, co2Char, gasChar, accChar, gyroChar, quatChar] =
      characteristics;
